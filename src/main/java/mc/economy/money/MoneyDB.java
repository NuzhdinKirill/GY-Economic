...
public class MoneyDB {
    ...
    public MoneyDB(GY plugin) {
        this.plugin = plugin;
    }

    public void init() {
        try {
            String dbPath = plugin.getDataFolder().getAbsolutePath() + "/money.db";
            connection = DriverManager.getConnection("jdbc:sqlite:" + dbPath);

            try (Statement stmt = connection.createStatement()) {
                stmt.execute("CREATE TABLE IF NOT EXISTS balances (uuid TEXT PRIMARY KEY, balance REAL DEFAULT 0)");
                plugin.getLogger().info("БД монет подключена: " + dbPath);
            }
        } catch (SQLException e) {
            plugin.getLogger().severe("Ошибка подключения к БД монет: " + e.getMessage());
        }
    }
